 ...
 * so voiced coach replies reuse the reference-audio playback path.
 */
@Singleton
class CacheFileConversationAudioPlayer @Inject constructor(
    ...
    override suspend fun playSpeech(bytes: ByteArray, contentType: String?) {
        ...
        val name = "conversation-speech-" + digest.joinToString("") { "%02x".format(it) } + ".$extension"
        val file = withContext(Dispatchers.IO) {
            val target = File(context.cacheDir, "conversation-speech/$name")
