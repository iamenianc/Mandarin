...
| --- | --- | --- |
| WF-1 | 4 MiB | two parts, 2 MiB decoded each |
| WF-2, WF-4, WF-7, WF-10 | 2 MiB | one part, 2 MiB decoded |
...
| `413` | `{ "error": "payload too large" }` or `{ "error": "audio part too large" }` | body or audio part over the workflow cap |
...
`/v1/chat` keeps its documented shape: oversize bodies return `413` and upstream failures
return `{ "error": "upstream error", "status": <upstream code> }`.
...
- **WF-3 cannot run Kokoro in the Worker.** No runtime synthesis provider is documented
  ...
  contract used here is provisional: `{ "input": "<pinyin or replyText>", "voice?",
  "language?" }` returning audio bytes. Build-time reference clips stay bundled (ADR 0006).
- **No streaming.** `docs/02-architecture.md` prefers WebSocket streaming for conversation
  latency (NFR-2); all routes are unary JSON requests.
- **No rate limiting yet.** ADR 0003 makes the Worker the rate-limiting boundary; only
  payload caps are implemented.
